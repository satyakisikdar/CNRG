...
def get_tree(lst, level, i, max_level):
    ...
    for idx in lst[level][i]:
        tree.append(get_tree(lst=lst, level=level+1, i=idx, max_level=max_level))

    return tree

def get_louvain_clusters(graph, partition=None,
                   weight='weight', resolution=1., randomize=False):
    dendo = generate_dendrogram(graph,
                                partition,
                                weight,
                                resolution,
                                randomize)
    level_clusters = deque()
    for clu_level in reversed(dendo):
        level_clusters.append(get_clusters(clu_level))

    level_clusters.append([[n] for n in sorted(graph.nodes_iter())])

    level_clusters.appendleft([list(range(len(level_clusters[0])))])

    level_clusters.appendleft([[0]])

    return get_tree(lst=level_clusters, level=0, i=0, max_level=len(level_clusters)-1)
...
def __one_level(graph, status, weight_key, resolution, randomize):
    ...
    new_mod = cur_mod
    # print('One level called')
    while modified and nb_pass_done != __PASS_MAX:
        # print('\nPass {}'.format(nb_pass_done + 1))
        cur_mod = new_mod
        modified = False
        nb_pass_done += 1

        for node in __randomly(graph.nodes_iter(), randomize):
            ...
            for com, dnc in __randomly(neigh_communities.items(),
                ...
                if incr > best_increase:
                    best_increase = incr
                    best_com = com
            # print('node: {} ({}) best neighbor: {}'.format(node, com_node, best_com))
